import { f as createComponent, r as renderTemplate, l as defineScriptVars, k as renderComponent, m as maybeRenderHead } from '../../chunks/astro/server_CJV_qxen.mjs';
import 'piccolore';
...
const $$Search = createComponent(async ($$result, $$props, $$slots) => {
  const allSiteFetch = await fetch(getAPIPath() + "api/siteData.json").then(
    (Response) => Response.json()
  );
  const allPanelsFetch = await fetch(getAPIPath() + "api/panels.json").then(
    (Response) => Response.json()
  );
  return renderTemplate(_a || (_a = __template(["", " <script>(function(){", `
    ...
    async function fetchData() {
        ...
        const url = \`https://smmsearch.netlify.app/api/panelServices/search_\${query.toLowerCase()}\`;
    ...
    // Init
    fetchData();
})();<\/script>`])), renderComponent($$result, "PanelLayout", $$PanelLayout, { "title": ` on ${allSiteFetch[0].SiteTitle}`, "metaDescription": `Search among Best SMM Panels, and SMM Services on ${allSiteFetch[0].SiteTitle}` }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="content-wrapper flex flex-wrap lg:justify-start justify-center gap-5 my-6 px-5"> <article class="flex flex-col gap-5 w-full"> <h1 class="searchResTitle text-2xl font-semibold"></h1> <div class="searchBox flex justify-center items-center"> <div class="wrapper w-1/2"> ${renderComponent($$result2, "SearchForm", $$SearchForm, { "searchPagePath": "/services/search" })} </div> </div> <!-- Filters --> <div id="FiltersElementsHolder" class="z-[1] w-full pt-4 gap-4 items-center flex-wrap justify-between shadow mt-5 rounded-md bg-[#7f7f7f] p-3 hidden"> <div> <label class="mr-2 font-medium text-white">Filter by Rating:</label> <select id="ratingFilter" class="border p-1 rounded text-black"> <option value="">All Ratings</option> <option value="5">⭐ 5 Stars</option> <option value="4">⭐ 4 Stars & above</option> <option value="3">⭐ 3 Stars & above</option> <option value="2">⭐ 2 Stars & above</option> <option value="1">⭐ 1 Star & above</option> </select> <div class="mt-6"> <label class="flex items-center gap-2 cursor-pointer select-none w-fit"> <input id="newArrivalFilter" type="checkbox" class="sr-only peer"> <div class="block relative bg-gray-300 w-11 h-6 p-1 rounded-full 
                                    before:top-[0.15rem]
                                    before:absolute before:bg-white before:w-5 before:h-5 before:p-1 before:rounded-full before:transition-all before:duration-500 before:left-1 peer-checked:before:left-[1.4rem] peer-checked:bg-blue-500"></div> <span class="text-sm text-white">Only Show Newest Arrivals</span> </label> </div> </div> <div> <label class="mr-2 font-medium text-white">Results per page:</label> <select id="resultsPerPage" class="border p-1 rounded text-black"> <option value="10">10</option> <option value="25">25</option> <option value="50">50</option> </select> </div> </div> <div class="services-table-holder text-center lg:w-full w-[89vw] overflow-auto z-[1] mx-auto mt-5 rounded-md bg-[#7f7f7f] p-3"> <!-- Loader --> <div id="loader" class="flex flex-col gap-2 justify-center items-center py-10"> <div class="animate-spin rounded-full h-10 w-10 border-4 border-white border-t-transparent"></div> <p id="loaderText" class="text-white text-sm">Please wait... 0% Loaded</p> </div> <!-- Table --> <div class="p-4 hidden" id="tableHeader"> <table class="w-full table-auto text-left shadow rounded"> <thead> <tr class="offerHeader w-full bg-[#000000b9] font-semibold text-white rounded-sm"> <th class="p-4 cursor-pointer lg:w-max-[500px]" data-sort="serviceName">Service Name ⬍</th> <th class="p-4 cursor-pointer" data-sort="serviceMin">Min Order ⬍</th> <th class="p-4 cursor-pointer" data-sort="serviceMax">Max Order ⬍</th> <th class="p-4 cursor-pointer" data-sort="serviceRate">Rate per 1000 ⬍</th> <th class="p-4 text-center">Panel</th> </tr> </thead> <tbody id="tableBody" class="text-white"></tbody> </table> </div> </div> <!-- Pagination --> <div id="PaginationElementHolder" class="p-4 justify-between items-center w-full hidden"> <button id="prevPage" class="bg-slate-700 text-white px-4 py-1 rounded disabled:opacity-50"><i class="fa-duotone fa-chevrons-left"></i> Previous</button> <span id="pageIndicator" class="font-semibold"></span> <button id="nextPage" class="bg-slate-700 text-white px-4 py-1 rounded disabled:opacity-50">Next <i class="fa-duotone fa-chevrons-right"></i></button> </div> </article> </div> ` }), defineScriptVars({ panelsData: allPanelsFetch }));
}, "/Users/niangamadou888/Documents/SMMsearch-frontend/src/pages/services/search.astro", void 0);

const $$file = "/Users/niangamadou888/Documents/SMMsearch-frontend/src/pages/services/search.astro";
const $$url = "/services/search";
